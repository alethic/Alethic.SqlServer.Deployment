...
namespace Cogito.SqlServer.Deployment.Tool
{
    ...
    /// <summary>
    /// Provides SQL instances for the given FSX instance names.
    /// </summary>
    [RegisterAs(typeof(SqlEnvironmentDbInstanceProvider))]
    [RegisterSingleInstance]
    public class SqlEnvironmentRemoteDbInstanceProvider :
        SqlEnvironmentDbInstanceProvider
    {

        class SqlSystemDbInstance : ISqlDbInstance
        {
            ...
            public SqlSystemDbInstance(string serverName, string dataSource)
            {
            ...
            {
                return new SqlConnection(CreateConnectionStringBuilder().ConnectionString);
            }
        ...
        const string SQL_EXPRESS_URI = @"https://download.microsoft.com/download/5/E/9/5E9B18CC-8FD5-467E-B5BF-BADE39C51F73/SQLServer2017-SSEI-Expr.exe";
        ...
        {
            // temporary file
            var f = Path.Combine(Path.GetTempPath(), "SQLServer2017-SSEI-Expr.exe");
        ...
        readonly SqlEnvironmentOptions options;
        readonly SqlInstanceMap map;
        readonly ConcurrentDictionary<string, Task<SqlSystemDbInstance>> instances = new ConcurrentDictionary<string, Task<SqlSystemDbInstance>>();
        ...
        public SqlEnvironmentRemoteDbInstanceProvider(
            IOptions<SqlEnvironmentOptions> options,
            SqlInstanceMap map)
        {
        ...
        /// <returns></returns>
        public override async Task<ISqlDbInstance> GetOrCreateInstanceAsync(string name)
        {
            return await await GetInstanceTask(name);
        }

        /// <summary>
        /// Gets the task scheduled to create the instance.
        /// </summary>
        /// <param name="name"></param>
        /// <returns></returns>
        async Task<Task<SqlSystemDbInstance>> GetInstanceTask(string name)
        {
            using (await SYNC.LockAsync())
                return instances.GetOrAdd(name, key => Task.Run(async () => await CreateInstance(key)));
        }
        ...
        /// <returns></returns>
        async Task<SqlSystemDbInstance> CreateInstance(string instanceName)
        {
            if (instanceName == null)
                throw new ArgumentNullException(nameof(instanceName));

            // pull out configured name
            var dataSource = map.GetOrDefault(instanceName);
            if (dataSource == null)
                throw new InvalidOperationException($"Could not resolve SQL instance name for {instanceName}.");

            // attempt to refresh server name
            var serverName = await TryGetServerName(dataSource) ?? dataSource;
            ...
            if (GetLocalServerNames().Any(i => host.Equals(i, StringComparison.OrdinalIgnoreCase)))
            {
                ...
                if (await TryGetServerName(dataSource) is string s)
                {
                    ...
                    return new SqlSystemDbInstance(s, dataSource);
                }
        ...
        async Task InstallSqlServer(string instanceName)
        {
            ...
            using (await SYNC.LockAsync())
            {
                ...
                {
                    var setup = Path.Combine(options.Setup, "setup.exe");
                    if (File.Exists(setup) == false)
                        throw new FileNotFoundException($"Unable to find {setup}.");

                    // run setup
                    var exitCode = await RunInstallAction(setup, instanceName);
                    if (exitCode != 0)
                ...
                else
                {
                    ...
                    var media = Path.Combine(Path.GetTempPath(), "SQLServer2017Media");
